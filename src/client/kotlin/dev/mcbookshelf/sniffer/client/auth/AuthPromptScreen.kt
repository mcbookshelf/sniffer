...
/**
 * Modal asking the player whether the debug client claiming their account may attach.
 *
 * @param requestId the pending request this screen answers
 * @param clientDescription what to show the player about the client asking
 * @param timeoutSeconds how long the player has to answer before the request is rejected on its own
 * @author theogiraudet
 */
class AuthPromptScreen(
    ...
    override fun removed() {
        // Closing the screen without answering counts as a rejection.
        if (!decisionSent) {
            decisionSent = true
            try {
                ClientPlayNetworking.send(AuthResponsePayload(requestId, false))
            } catch (_: Exception) {
                // The connection may already be going down.
            }
        }
        super.removed()
    }
