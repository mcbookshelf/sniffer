...
| Expression | ✓ | Pause only when the given expression holds. Here, an expression is a Minecraft command read on its success channel, the same thing `/breakpoint <command>` takes. It runs with the execution context of the paused command, and its output is silenced. A command that does not parse leaves the breakpoint unverified, with the parse error as its message. |
